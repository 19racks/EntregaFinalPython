...
def addblogpost(request):
    if request.method == 'POST':
        ...
        if myForm.is_valid():
            myForm.save()
            return render(request, "index.html", {'message': "New post was added"})
    else:
        myForm = BlogPostForm()
...
def deletepost(request, post_id):
    ...
    blogpost.delete()
    blogposts = BlogPost.objects.all()
    return render(request, 'index.html', {'blogposts': blogposts, 'message': "The post has been deleted."})
